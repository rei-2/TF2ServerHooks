...
public:
	virtual int GetModelIndex(const char* name) const = 0;
	virtual int GetModelClientSideIndex(const char* name) const = 0;
	virtual bool RegisterModelLoadCallback(int modelindex, IModelLoadCallback* pCallback, bool bCallImmediatelyIfLoaded) = 0;
	virtual void UnregisterModelLoadCallback(int modelindex, IModelLoadCallback* pCallback) = 0;
	virtual bool IsDynamicModelLoading(int modelIndex) = 0;
	virtual void AddRefDynamicModel(int modelIndex) = 0;
	virtual void ReleaseDynamicModel(int modelIndex) = 0;
	...
	virtual bool IsModelVertexLit(const model_t* model) const = 0;
	virtual bool IsTranslucentTwoPass(const model_t* model) const = 0;
	virtual void RecomputeTranslucency(const model_t* model, int nSkin, int nBody, void /*IClientRenderable*/* pClientRenderable, float fInstanceAlphaModulate) = 0;
	virtual int	GetModelMaterialCount(const model_t* model) const = 0;
	virtual void GetModelMaterials(const model_t* model, int count, IMaterial** ppMaterials) = 0;
	virtual void GetIlluminationPoint(const model_t* model, IClientRenderable* pRenderable, const Vector& origin, const QAngle& angles, Vector* pLightingOrigin) = 0;
	virtual int GetModelContents(int modelIndex) = 0;
	virtual const char* GetModelKeyValueText(const model_t* model) = 0;
	virtual bool GetModelKeyValue(const model_t* model, CUtlBuffer& buf) = 0;
	virtual float GetModelRadius(const model_t* model) = 0;
	...
	virtual int GetModelSpriteWidth(const model_t* model) const = 0;
	virtual int GetModelSpriteHeight(const model_t* model) const = 0;
	virtual const studiohdr_t* FindModel(const studiohdr_t* pStudioHdr, void** cache, char const* modelname) const = 0;
	virtual const studiohdr_t* FindModel(void* cache) const = 0;
	virtual virtualmodel_t* GetVirtualModel(const studiohdr_t* pStudioHdr) const = 0;
	virtual byte* GetAnimBlock(const studiohdr_t* pStudioHdr, int iBlock) const = 0;
	virtual void SetLevelScreenFadeRange(float flMinSize, float flMaxSize) = 0;
...
{
public:
	virtual int RegisterDynamicModel(const char* name, bool bClientSideOnly) = 0;
	virtual const model_t* GetModel(int modelindex) = 0;
	virtual const model_t* FindOrLoadModel(const char* name) = 0;
	virtual void OnDynamicModelsStringTableChange(int nStringIndex, const char* pString, const void* pData) = 0;
...
MAKE_INTERFACE_VERSION(CModelInfoServer, ModelInfoServer, "engine.dll", "VModelInfoServer004");